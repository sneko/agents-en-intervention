...
export function SignInForm({ prefill }: { prefill?: SignInPrefillSchemaType }) {
  ...
  const sessionEnd = searchParams?.has('session_end') || false;
  const registered = searchParams?.has('registered') || false;
  ...
  const onSubmit = async ({ username, password, rememberMe }: any) => {
  ...
  return (
    <BaseForm handleSubmit={enhancedHandleSubmit} onSubmit={onSubmit} control={control} ariaLabel="se connecter">
      ...
      <Grid item xs={12}>
        <TextField label="Nom d'utilisateur" {...register('username')} error={!!errors.username} helperText={errors?.username?.message} fullWidth />
      </Grid>
      <Grid item xs={12}>
        <TextField
